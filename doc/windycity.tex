% Last modified: Sun 01 Nov 2020 07:57:06 PM CST
...
The citations show Windy City's default output, which matches the
standard format of citations within \textit{CMOS}. A work's first
citation is similar to its entry in the bibliography. It includes all
or most of its bibliographic information. Subsequent citations are
...
example from \textit{CMOS} \ref{14.34}:\footnote{Switching formats
...
\noindent In the short format, a work's first citation
gives short names and titles and omits all other publication
information. Consecutive citations of a work may omit the title or, as
in the eleventh note, where the title is the key mark of distinction,
the author's name. To use this format, start \biblatex with the
preamble option \opt{short}. See section \ref{preops} for more
information.
...
shorten the author's name of a work's first citation if the previous
...
\noindent Similar to \opt{isbn} but for printing ISSNs.
...
\opt{short} affects consecutive citations of a work on the same page,
never in reference to a citation on a previous page. As with
\textit{ibid.}, this feature isn't required by \textit{CMOS}, but it
prevents readers from having to look at another page to find the title
of a citation.
...
\bibfield{shorttitle} field, if available. Either way, the short
format in those contexts is no different from the default.
...
\textit{CMOS} \ref{14.57}. The sources mentioned in a passage are
cited in the standard, long format with shortened names:
...
tell Windy City to shorten names in the long format. The preamble
option \opt{idemtracker} helps with consecutive citations of the same
author, but that's no use when you want to shorten names because
you've mentioned them in the text.

Nevertheless, there are at least three ways to get this output with
standard commands:
...
commands to suppress the author's position while you handle it
manually---but they do work.

With \cmd{idemcites} and \cmd{footidemcites}, you have simpler
alternatives that make your intentions clearer and your file easier to
read:
...
\textit{Chaucer Life-Records} prints this \bibfield{editoraddon}:
``from materials compiled by John M. Manly and Edith Richert, with the
assistance of Lilian J. Redstone et al.''
