% Last modified: Thu 06 Dec 2018 03:14:39 PM CST
...
implies \opt{firstshort} but goes beyond it to give a short format to
...
the style lists editors first. However, entries with the option
...
\begin{verbatim}
   \cite{frankfurt1969} \citerp[1--10]{frankfurt1988.1}
\end{verbatim}
...
example from \textit{CMS} \ref{14.105} uses it: In the work's database
entry, \bibfield{editoraddon} contains ``from materials compiled by
...
number. For an example, see Section \ref{multivolume}.
...
\BibTeX\ and \biblatex, though it seems harmless enough.} You may
choose either \bibtype{InBook} or \bibtype{BookInBook} entries have
the same format. If an entry belongs to a type other than the ones
listed above, the style processes it as a book.
...
below is from \textit{CMS} 14.264:
...
\begin{verbatim}
   @Misc{roosevelt1959,
     ...
     usera = {convocation speech, Ball State Teacher's College,
              ...
              reel-to-reel tape, MPEG copy, 1:12:49},
...
titles of the volumes differ, so cross-referencing alone won't suffice
to turn multivolume collections into single works.
...
the \bibtype{Collection} is true of the volume.
...
use \bibfield{note} for holding \textit{Part I} and \textit{Part II}.
Citing particular works from each volume is then a straightforward
matter of making \bibtype{InCollection} entries for those works and
...
volume number? Try adding \bibfield{maintitle} to \bibtype{Collection}
along with a new field, \bibfield{volumea}, for the volume of
...
%immediately after the newspaper's title and not in parentheses. The
%problem stems from the style using many of the same macros to format
%every article, regardless of whether its source is a journal,
%magazine, or newspaper.

%For reference lists, unsigned reviews have no plausible place to put
%the publication year but where the default format would put it. The
%easiest solution is to give unsigned reviews the same format in
%reference lists and bibliographies.

% FIX: Unsigned reviews don't work with the \opt{short} or
% \opt{firstshort} preamble options. In the default format, only the
% first citation close to correct.

% FIX: Presumably, CMS places 'nos. 23–24' after the date because the
% date is a more important part of a magazine's or newspaper's
% citation.
...
At present, Windy City has difficulty with some reference works.
Exceptions like the ones below show full publication information in
