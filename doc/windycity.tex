% Last modified: Tue 26 Mar 2019 07:45:00 PM CDT
...
\noindent If you need to install Windy City on your system, you should
have several options of where to put these files. One option is to
locate \biblatex on your system and copy Windy City's files into their
...
option is to figure out where your distribution of \LaTeX keeps
\biblatex's style files. Under
\href{http://www.tug.org/texlive}{TeXLive}, the directory structure
should look like this:

\begin{itemize}[before=\small,label=]
\item \ldots\path{/texmf-dist/tex/latex/windycity/windycity.dbx}
\item \ldots\path{/texmf-dist/tex/latex/windycity/bbx/windycity.bbx}
\item \ldots\path{/texmf-dist/tex/latex/windycity/cbx/windycity.cbx}
\item \ldots\path{/texmf-dist/tex/latex/windycity/lbx/american-windycity.lbx}
\end{itemize}

\noindent Yet another option is to install Windy City in your local
\textit{texmf}. Whatever location you choose, remember to update your
\textit{texmf} file name database after copying in the files.

Next, tell \biblatex to load Windy City with the load-time option
\opt{style}:
...
As Windy City has grown to support more citations, it has had to rely
on a growing number of data fields that aren't standard with \BibTeX
or \biblatex. It also uses some standard fields in perhaps unexpected
ways.

\subsubsection{Additional Data Fields}
\label{datafields}

Some of the additional or nonstandard data fields that Windy City uses
are for its internal handling of cross-referencing. Those fields
aren't listed below, as they're not meant for use in a bibliography
database. Users can ignore them. The rest store information that's
...
Windy City makes limited use of the fields \bibfield{issue} and
\bibfield{type}, and how it uses them probably isn't obvious from
entries in \file{windycity.bib}. A few comments should clear up
confusion.

\begin{marglist}

\item[issue] Windy City uses this field with the \bibtype{article},
\bibtype{online}, and \bibtype{review} entry types to record the
...
\begin{verbatim}
   issue = {Fall},
   date = {2013},% alternatively, use the 'year' field
\end{verbatim}
...
\textit{phdthesis}. For the former, use the value \textit{mathesis}.
No \bibfield{type} field is needed for the \bibtype{mathesis} and
