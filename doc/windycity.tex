% Last modified: Wed 07 Apr 2021 08:31:21 PM CDT
...
\bibfield{translatora}, and \bibfield{translatorb}). For best results,
use the name lists in order---\bibfield{editor} before
\bibfield{editora}, \bibfield{translator} before
\bibfield{translatora}, and so on---always starting with the editors
and translators of a \bibfield{title} before those of an
\bibfield{issuetitle}, \bibfield{maintitle}, or \bibfield{series}.

In most cases, you'll assign editors and translators to a
\bibfield{title}. Since it's the default assignment, you can do it
...
Windy City supports a second tier of editors after titles. This
includes \bibfield{title}, \bibfield{issuetitle},
\bibfield{maintitle}, and \bibfield{series}. In your bibliography
database, make sure to list editors in the proper order:
\bibfield{editor} precedes \bibfield{editora}, and \bibfield{editora}
precedes \bibfield{editorb}. Second tier editors never print in the
author's position, even when the first does. They always print after
the relevant title.
...
ambiguity about which title they're responsible for. Both editor name
lists fall within \bibfield{carter2012}, so they can't apply to
``Pitch and Transposition,'' and the only title left is \textit{A
Performer's Guide}.

As the examples above also help to show, the style supports a variety
of editorial roles beyond just editor and translator. Those roles are
compiler, expander, reviser, and updater. To assign them, you need to
use the \bibfield{editortype} and \bibfield{translatortype} fields.
Below are some bibliography database entries for examples in
\textit{CMOS} \ref{14.103} and \ref{14.142}:
...
expander, reviser, and updater), you may assign any pair of those
roles to an editor or translator name list. A work's editor, for
...
\ref{table:ed} lists all editor and translator types.
...
\ref{table:ed}, we need to turn to the bibliography. The first example
below shows the output of \textit{reved} in the \bibfield{editortype}
...
need to use editor or translator type fields with values
...
option \opt{swaptrans}, which would give ``translator and editor''
...
\textit{extrans}? If the editors and translators of a title are the
same, but the type fields differ, Windy City reads the editor type
field and ignores the translator type field.

Three more values for type fields allow you to assign editors and
translators to higher level titles: \textit{issuetitle},
\textit{maintitle}, and \textit{series}. Here's an example from
...
\bibfield{editor} to \bibfield{series}, the style would assign it to
\bibfield{title}. Much the same goes for translators: Assign them to a
