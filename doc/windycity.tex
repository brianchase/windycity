% Last modified: Mon 07 Jan 2019 07:25:36 PM CST
...
In some examples, \textit{CMS} shows two ways of citing works that
belong to collections: with publication information for the volume
preceding that of the collection or \textit{vice versa}. (See especially
\textit{CMS} \ref{14.119}, \ref{14.121}, and \ref{14.122}, and
compare \ref{14.144} and \ref{15.41}.) In the standard format, it's
unclear whether \textit{CMS} has a preference. But the examples
suggest that, if one must be the default, it's to give priority to the
collection. For the alternative, use \opt{volfirst}. The preamble
option \opt{volfirst} has a global effect. It changes the format of
all relevant citations. The entry option \opt{volfirst} changes the
format on a case-by-case basis (see Section \ref{entryops}).

Bear in mind, some works need this option. Those cases are limited to
citations of volumes in collections in which different volumes have
different authors. Not using \opt{volfirst} in such a case would
create the impression that the author of the volume is also the author
of the collection. For an example, see \textit{CMS} \ref{14.122}.
...
translators, and compilers. This section shows how to take advantage
of it.

\subsection{Placing Editors, Translators, and Compilers}
\label{edtranspos}

Normally, the style prints editors' names first. But if translators
...
names first. For that, use the entry option \opt{transfirst} (see
Section \ref{entryops}). Compare:
...
name of the editor in the author's position. A similar switch happens
if a work has no author but has a translator. The following are
examples from \textit{CMS} \ref{14.103}:
...
What happens if a work has no author but has editors and translators?
Since the style gives priority to editors, it defaults to printing the
...
example of that from \textit{CMS} \ref{14.104}:
...
Windy City associates the \bibfield{editor} and \bibfield{translator}
fields with the lowest level title within the scope of an entry. For
most sources, then, no more work is necessary than just populating the
\bibfield{editor} and \bibfield{translator} fields of a database
entry. Below is the entry for an example in \textit{CMS} \ref{14.104},
printed in the previous section:
...
title. In this example from \textit{CMS} \ref{14.30}, an essay is
...
collection. Thus, \bibfield{translator} must fall within the scope of
...
collection, \textit{The Renaissance Philosophy of Man}, not with the
essay.
...
it.} As for \textit{compiler}, remember that it's a kind of editor.
So, if you need to add a compiler to an entry, add the compiler's name
to an \bibfield{editor} field, assuming it's free, then add an
\bibfield{editortype} field with the value \textit{compiler}. It may
help to see the bibliography database entry for an example in
...
\noindent If there were no \bibfield{editortype} associating the
editor with the collection, the style would associate the editor with
the volume.
...
clever than the previous paragraph would suggest. But if you follow
that rule of thumb, as well as examples in \file{windycity.bib}, you
won't need to worry about editors' names appearing where they don't
belong.

\section{Collections as Single Works}
\label{multivolume}
...
option \opt{volfirst} (see Section \ref{entryops}). Without it, the
resulting entry would give the impression that he wrote every volume
in the collection, among them the first. With Donne, on the other
hand, you may choose one format or the other.
