% Last modified: Mon 24 Feb 2014 11:20:52 AM EST
...
\email{brian@hesperusisphosphorus.com}
\version{0.8}
\date{\today}
\maketitle
...
\nfootnote{Copyright \textcopyright\ 2014 Brian Michael Chase. Under
...
in the bibliography database includes a \bibfield{shorttitle} or, if
applicable, a \bibfield{shortbooktitle} (see Section
\ref{datafields}), the style will print one of them, giving preference
to \bibfield{shorttitle}. If neither field is available, the short
form is the work's title minus its subtitle.

If a work uses an abbreviation---a \bibfield{shorthand} in the
terminology of \biblatex---the \bibfield{shorthand} replaces
...
a \bibfield{shorthand} announces it at the end of the citation, as in,
``Hereafter, \textit{CMS}.'' See \ref{sh}, near the end of this
document, for the output of \cmd{printshorthands}.
...
default for \opt{mincrossrefs} is \textit{2}. This means that the
...
whole, use \opt{collsonly}. This option sets \opt{mincrossrefs} to
\textit{1} and then filters from the bibliography all \bibtype{inbook}
entries that cross-reference other entries and all
\bibtype{incollection} entries that cite chapters of a
...
\item[nolos] By default, every work with a \bibfield{shorthand}
receives an entry in the bibliography. If you wish to exclude them,
say, to avoid duplication with the output of \cmd{printshorthands},
use \opt{nolos}.
...
\opt{reflist} to maintain consistency with \textit{CMS}. You can
accomplish the same effect by giving the option \opt{env=reflist} to
...
\begin{itemize}
\item[N] \fullcite{crow1966b}

\item[B] \bibentry{crow1966b}
\end{itemize}
...
editors first. However, entries with the option \bibfield{transfirst}
reverse this order: Their translators print first. If a work's
translators and editors are identical, using \bibfield{transfirst}
...
\cmd{bibentry} and \cmd{refentry}, some discrepancies with
...
% No need to mention, but this command differs from \cmd{fullcite}
% only in that it does not print \bibfield{shorthands}.
%\cmditem{fullcitens}[prenote][postnote]{key}
...
%This command is strictly for the style's internal use when processing
%\bibtype{article} and \bibtype{review} entries. If those entries have
%the data field \bibfield{review}, the style uses this command to cite
%the \bibfield{entrykey} in that field. For more on reviews, see
%\bibfield in Section \ref{datafields} and \S\ref{14.215}.
...
\item[bookyear] This field is for the style's internal use. Do not use
it in a bibliography database.
...
% NOT IN USE:
%\item[review] This field, for \bibtype{article} or \bibtype{review}
%entries, holds the \bibfield{entrykey} of a work under review. An
%example appears below, in \S\ref{14.215}.

\item[seriesaddon] This field is for additional information about a
series, such as \textit{2nd ser.} and \textit{n.s.}. For examples, see
\S\ref{14.132} and \S\ref{14.195}.
...
% NOT IN USE:
%\item[translatortype] The style recognizes just two types of
%translators, those of a \bibfield{title} and those of a
%\bibfield{booktitle}. Accordingly, the style looks for just two
%values of \bibfield{translatortype}: \textit{title} and
%\textit{booktitle}. By default, the style treats all translators as
%those of a \bibfield{title}. Therefore, you only need to use
%\bibfield{translatortype} to indicate the translator of a
%\bibfield{booktitle}. For more information on how the style handles
%titles, see Section \ref{authtitles}. For more information on how
%the style handles translators, see Section \ref{edtrans}.
...
volume of the collection has the same publication year.\footcite[See
the examples of short citations in \S14.29 and
...
What matters is that the publication year of the volumes matches the
...
correct output. Adding more information has no effect. Thus, you could
change the entry to:
...
using \bibfield{part} is incorrect, since its function is to indicate
parts of a volume, not volumes of a collection. What should you do?
...
entries. The output is at least adequate:
...
\bibfield{maintitle} and reserve \bibfield{volume} for the volume of
...
\typeitem{unpublished} Aliases: none
...
\bibtype{inbook} or \bibtype{bookinbook} entry type. You may choose
either; \bibtype{inbook} or \bibtype{bookinbook} entries have the same
format. If an entry belongs to a type other than the ones listed
above, the style processes it as a book.

For unusually complicated ci\-ta\-tions---at any rate, too complicated
for the style---consider using the \bibtype{misc} entry type. The
...
translator, as in this example, also from \S\ref{14.87}:
...
compilers and translators, the style will print the compilers' names
first, unless you use \opt{transfirst}. For more on compilers, see
...
\begin{itemize}
\item[N] \fullcite[133--34]{adams1868}

\item[B] \bibentry{adams1868}
...
% The \bibfield{doi} won't wrap. The problem is that, if I want a proper
% link to the document, I need to wrap \bibfield{doi} in 'href' with the
% initial part of the address. But, it seems, \cmd{biblatex} wraps
% links in 'url', not 'href'.
...
% CHECK: Is it possible to add this?
...
% No need to add this:
%\subsubsection{Magazine article titles}
% 14.201

% FIX: Not possible. Add a field for 'department'?
...
% FIX: Not possible. Add fields for this?
...
% Not possible: too many oddities in how CMS format this:
%\begin{itemize}
...
% FIX: Not possible. Perhaps use 'type' field to identify an entry as
% citing a dictionary or encyclopedia and cite accordingly.
%\subsubsection{Dictionaries and encyclopedias}
% 14.247

% FIX: Not possible. See previous note.
...
does not automatically italicize shorthands, as whether you should
italicize a shorthand depends on whether you should italicize the
work's title (\S14.55). See the entry for \textit{CMS} in
\file{windycity.bib}.\\}%
...
\printbibliography[notkeyword=exclude,prenote=bib]
