% Last modified: Fri 02 Nov 2018 06:40:21 PM CDT
...
and bug fixes in the years since. Current releases nonetheless require
\biblatex version 3.8 or later.
...
defaults for \opt{mincrossrefs} and \opt{minxrefs} is \liningnums{2}.
...
for the collection as a whole, use \opt{collsonly}. This option sets
\opt{mincrossrefs} and \opt{minxrefs} to \liningnums{1} and then
filters from the bibliography all \bibtype{inbook} entries that
cross-reference other entries and all \bibtype{incollection} entries
that cite chapters of a cross-referenced book. Thus, it has no effect
on many \bibtype{incollection} entries, such as articles in books,
...
\noindent The 15\textsuperscript{th} edition of \textit{The Chicago
Manual of Style} shows this citations of an event spanning several
days:\footcite[\S17.215]{chicago2003}
...
does not automatically italicize shorthands, as whether you should do
so depends on whether you should italicize the work's title
(\S14.55).\\}%
