% Last modified: Sun 14 Mar 2021 05:26:13 PM CDT
...
standard (see section \ref{preamble}). For another variant, try
\opt{shortafter}, which gives the same output as \opt{short} but with
long first citations. Also with standard citations, the preamble
option \opt{idemtracker} shortens the author's name of a work's first
citation if the previous citation is of the same author (see section
\ref{preamble}). The entry option \opt{noauth} omits the author's name
altogether (see section \ref{entry}). And the \bibfield{shorthand}
field allows you to set an abbreviation to stand in place of the
author's name, the work's title, and other elements of a citation (see
sections \ref{otherfields} and \ref{14.59}).
...
editors and translators of a \bibfield{title}, the option
\opt{swaptrans} reverses this order: Their translators print first. If
a work's translators and editors are the same, \opt{swaptrans}
...
\opt{swaptrans:issue}, \opt{swaptrans:main}, or
\opt{swaptrans:series}, respectively. For more information, see
...
As a result, options that may change where \bibfield{editor} prints,
such as \opt{swaptrans} and \opt{swapauth}, may determine whether this
field prints.
...
The style supports a second tier of editors after \bibfield{title}
(with some exceptions) and after every \bibfield{issuetitle},
\bibfield{maintitle}, and \bibfield{series}. With \bibfield{title}, a
second tier of editors is supported in most cases. Exceptions occur
only when another \bibfield{title} is available to take the
assignment. Among other possibilities, this could happen with works
cross-referenced to collections: The collection itself could have two
tiers of editors, but works cross-referenced to it, such as articles
and books, could only have one.

If the style recognizes a second tier of editors, \bibfield{editor}
precedes \bibfield{editora}, and \bibfield{editora} preceedes
\bibfield{editorb}. Further, the second editor always prints after
\bibfield{title}, even when the first occupies the author's position.
...
ambiguity about which title they're responsible for. Neither is an
editor of ``Pitch and Transposition,'' so both must be editors of
\textit{A Performer's Guide to Seventeenth-Century Music}. If there
were another \bibfield{title}, the style would assign
\bibfield{editor} to \textit{A Performer's Guide} and
\bibfield{editora} to the next one.

As these exaples also show, the style supports editorial roles beyond
just editor and translator. Those roles are compiler, expander,
reviser, and updater. To assign them, you need to use the
\bibfield{editortype} field. Below are some bibliography database
entries for examples in \textit{CMOS} \ref{14.103} and \ref{14.142}:
...
In addition to the six basic roles (editor, translator, compiler,
expander, reviser, and updater), you may assign names to any pair of
them. A work's editor, for example, can also be its translator,
compiler, expander, reviser, or updater---but no more than one of
these. And just as you can swap the order of the editor and translator
roles (again, see section \ref{edtranspos}), you can swap all other
combinations, say, to identify a work as \textit{compiled and edited
by} Jane Doe rather than \textit{edited and compiled by} her. Simply
change the value of \bibfield{editortype}. Table \ref{table:ed} lists
these values.
...
need to use the editor or translator type fields with the values
\textit{editor} or \textit{translator}. The name fields alone set the
correct assignment.

Much the same holds when the editor and translator of a
\bibfield{title} are the same. The usual way to set this is with
identical name fields:
...
\noindent This gives a default assignment of ``editor and translator''
(\textit{edtrans} from Table \ref{table:ed}), unless you use the entry
option \opt{swaptrans}, which defaults to ``translator and editor''
(\textit{transed} from Table \ref{table:ed}). No editor or translator
type field is necessary.

When type fields are required, Windy City gives you options for how to
structure your bibliography entries: If you like, you can set the
...
or with both name fields and both type fields. For example, to set
Jane Doe as the compiler and translator of a \bibfield{title}, Windy
City treats all five options below as equivalent:
...
translator type fields may conflict, that their contents may differ
when, in reference to the same persons, they should be identical. In
those cases, Windy City ignores the translator type field.

Three more values for editor and translator type fields allow you to
assign editors and translators to higher level titles:
\textit{issuetitle}, \textit{maintitle}, and \textit{series}. Here's
an example from \textit{CMOS} \ref{14.123}:
...
\noindent If there were no \bibfield{editortype} assigning
\bibfield{editor} to the series, the style would assign it to
\bibfield{title}. Much the same would go for a translator: You'd need
a \bibfield{translator} field with that person's name and a
\bibfield{translatortype} field with the value \textit{series}.

All combinations of editors and translators are available to
...
and for any title at a higher level than \bibfield{title}, you need to
use an editor or translator type field (\bibfield{editortype} for
\bibfield{editor} and \bibfield{translatortype} for
\bibfield{translator}). Combined with Windy City's support for citing
articles, chapters, books, and other works within books, this scheme
allows for several sets of editors per work.
...
and translators of different titles: \opt{swaptrans} (for
\bibfield{title}), \opt{swaptrans:issue} (for \bibfield{issuetitle}),
\opt{swaptrans:main} (for \bibfield{maintitle}), and
\opt{swaptrans:series} (for \bibfield{series}). This level of control
...
For simplicity, let's focus on \opt{swaptrans}. Compare:
...
and updaters, you need to put a value in \bibfield{editortype} anyway.
