% Last modified: Thu 14 Feb 2019 04:04:01 PM CST
...
Windy City's support for translators is more limited. Unless a work's
bibliography database entry has a \bibfield{translatortype} field with
the value \textit{maintitle}, Windy City associates the names in
\bibfield{translator} with the entry's lowest level title.
...
By contrast, here's a case where \opt{swapvol} might not seem
desirable:
...
titles that Windy City can use to sort them correctly. Regardless,
\opt{swapvol} affects them, too. In \textit{CMS}, \ref{14.120} gives
an example of one:
...
work with \opt{swapvol}? Below is the entry (or entries) for an
earlier example:
