% Last modified: Wed 28 Apr 2021 07:48:29 PM CDT
...
their respective options below. See also section \ref{edtranspos}.
...
\bibfield{swaptrans} also affects those of a \bibfield{booktitle} and
\bibfield{bookbooktitle}. See section \ref{edtranspos}.
...
section \ref{edtypes.other}). The rest store information that's
...
\opt{swaptrans} entry option (see section \ref{edtranspos}). Within
each set, the style prints the first list that meets its criteria for
that context. Apart from exceptions discussed below, then, you can
...
In the examples below, the editor name lists print correctly because
there's no ambiguity about which title they're responsible for. Both
fall within the same entry, \bibfield{carter2012}, where their default
assignment is \textit{A Performer's Guide}. Citing a child entry,
\bibfield{myers2012}, causes the editor name lists to print together
after the collection's title, starting with \bibfield{editor}, whereas
citing the parent causes \bibfield{editor} to print in the author's
position.
...
example below shows the effect of \textit{reved}, the second of
\textit{revup}.
...
With the help of strings from Table \ref{table:ed}, all editor and
...
editor and translator type fields.

An example of double cross-referencing will set us up to use both:
...
reverse the order (see \textit{CMOS} \ref{14.104}). To do this, the
style provides entry options for every title that accepts editors and
translators. This means that if a work has multiple levels of titles
with multiple pairs of editors and translators, you may swap positions
of any pair without affecting the others.

For simplicity, let's focus on \opt{swaptrans}, the option that
suffices for most purposes. It applies to the editors and translators
of works other than an \bibfield{issuetitle}, \bibfield{maintitle}, or
\bibfield{series}. Compare:
...
options allow you to pick among these titles so as to swap whichever
set of editors and translators you want. Here's an example to showcase
all three options:
...
\begin{verbatim}
   @InCollection{doe2017,
     ...
     title = {An Essay},
     editor = {White, Amy},
     translator = {Jones, David},
     pages = {18–35},
     crossref = {jones2017}
   }
   @InBook{jones2017,
     title = {A Volume of Essays},
     editor = {Jones, David and White, Amy},
     translator = {Jones, David},
     volume = {1},
     crossref = {white2017}
   }
   @Collection{white2017,
     editor = {White, Amy and Jones, David and Rogers, Mary},
     translator = {Jones, David and Rogers, Mary},
...
To swap the first pair of editors and translators, use
\opt{swaptrans:title}:
...
To swap the second pair of editors and translators, use
\opt{swaptrans:booktitle} (or the short form \opt{swaptrans:book}):

\begin{citebib}
\item \cite{doe2017b}
\end{citebib}

To swap the third pair of editors and translators, use
\opt{swaptrans:bookbooktitle} (or the short form
\opt{swaptrans:bookbook}):
...
\opt{swaptrans:issuetitle}, \opt{swaptrans:maintitle}, and
