% Last modified: Mon 26 Nov 2018 01:17:09 PM CST
...
\opt{ibidtracker} to true, as by default it uses \textit{ibid.} to
...
\opt{ibidtracker} to false, along with the option \opt{ibid},
...
native preamble option \opt{ibidtracker}.
...
in notes. As a result, citations to the work always begin at the
title's position, though as usual, in the bibliography, the work's
entry begins with the author. To bypass the author's position on a
case-by-case basis, rather than globally, use \cmd{cite*} or
\cmd{footcite*}, described in Section \ref{citecmds}.
...
Like \cmd{cite*}, described above, this command suppresses the
author's position of a note. Otherwise, it functions like the native
\cmd{footcite} command.
...
information about a work. The following example comes from
\textit{CMS} \ref{14.89}:
...
\noindent In the work's database entry, \bibfield{editoraddon}
contains ``from materials compiled by John M. Manly and Edith Richert,
with the assistance of Lilian J. Redstone et al.''
...
\begin{itemize}
\item[N] \fullcite{strauss1962}

\item[B] \bibentry{strauss1962}
\end{itemize}
