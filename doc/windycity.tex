% Last modified: Sun 20 Jan 2019 03:19:00 PM CST
...
\noindent Whereas entries in a bibliography always give a collection's
total number of volumes, doing so in notes is optional (see
\textit{CMS} \ref{14.118}). In long citations, Windy City prints the
total in a note when the \bibfield{postnote} field is empty. This
approach is necessary to give the full form of a citation to
correspond with that given in the bibliography. But when the
\bibfield{postnote} contains information, such as a page reference,
Windy City's default is not to print the total number of volumes. Use
\opt{listvols} when you want it to. Below is an example from
\textit{CMS} \ref{14.59}:
...
translator, use \opt{swapauth}. This option works with the
\bibtype{book}, \bibtype{collection}, and \bibtype{inbook} entry
types. For more information, see Section \ref{edtranspos}.
...
\subsection{The Structure of Citations}
\label{collorder}
...
a \bibtype{collection} entry, is subject to \opt{swapvol} if it has a
...
A \bibtype{book} is subject to \opt{swapvol} if it has
...
\bibtype{collection} and only intend to cite one volume. But you risk
trouble if the volumes of the collection have different publication
dates, as they do for \textit{The Papers of Martin Luther King, Jr}.
If the publication information for the volume goes last, the
bibliography and long citation should list the publication date as
\textit{2014}. If the publication information for the collection goes
last, that date is \textit{1992–}. A \bibtype{book} entry, however,
has just one field for a publication date, so using \opt{swapvol} on
\bibfield{carson2014} would print \textit{2014} when the correct date
is \textit{1992–}. Otherwise, the \bibtype{book} and \bibtype{inbook}
...
which in turn requires you to use the \bibtype{inbook} approach
described above.
...
example---one that prints the announcement in a separate
sentence---see the first citation of \textit{CMS} in this document's
introduction.
...
The problem Farwell's citation is with the field
\bibfield{howpublished}, which seems like the best choice to contain
\textit{microfiche} but which, as must happen in other cases, prints
after the \bibfield{postnote}, \textit{p. 67, 3C12}.
