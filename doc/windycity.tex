% Last modified: Wed 16 Jan 2019 05:35:56 PM CST
...
option, then, \opt{volfirst} is for changing individual citations in
the default format.
...
\opt{volfirst} preamble option, which will affect all collections.
More likely, though, you'll want to give priority to the volume when
citing some works but not others. That's why there's a \opt{volfirst}
...
\opt{volfirst}, on the other hand, the format looks more familiar:
...
Another reason to prefer \opt{volfirst} with a work like Barrow's is
that, in the default format, they don't sort correctly in a
bibliography or reference list without help, such as by adding
\bibfield{sortname} fields to their bibliography database entries.
Otherwise, Windy City will sort them by the author's name. Recall that
the entry option \opt{swapauth} has the same problem (see Section
\ref{edtranspos}). Here, too, there's seemingly no way in \biblatex
for an entry option to determine the sort order of a work or for a
single sorting template to anticipate every format that's possible
under this style.

How does Windy City determine which entries in a bibliography database
are affected by \opt{volfirst}? Below are the entries for an earlier
example:
...
% The point of '\newrefcontext' below is to work around the 'sortname'
% for barrows1959. As explained in \ref{collorder}, works like
% barrows1959 sort incorrectly in the default format but correctly
% with the option 'volfirst'. The problem with setting 'volfirst' is
% that you can't work around it to demonstrate the default format in a
% bibliography.
