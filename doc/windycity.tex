% Last modified: Sun 31 Mar 2019 06:11:58 PM CDT
...
the text. Use it with a comma-separated list of entry keys to add
those particular works. Use it with an asterisk to add every work in
every \file{bib} file that you've loaded with \biblatex.

By default, Windy City sets the \opt{autocite} preamble option to
\opt{footnote}, which makes \cmd{autocite} equivalent to
\cmd{smartcite}, which in turn functions like different citation
commands depending on the context. In Windy City, \cmd{smartcite} is
equivalent to \cmd{footcite} in a document's body and \cmd{cite} in
footnotes and endnotes. Thus, \cmd{autocite} is, too. If you set the
\opt{autocite} preamble option to \opt{inline}, \cmd{autocite} is
equivalent to \cmd{parencite}. If you set it to \opt{plain},
...
Like \cmd{smartcite} but, like \cmd{cite*}, suppresses the author's
position of a note.
