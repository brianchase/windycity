% Last modified: Sun 08 Nov 2020 03:05:14 PM CST
...
\opt{ibid} together give the following:\footnote{As explained in
section \ref{preops}, Windy City won't print \textit{ibid.} in
reference to a citation on a previous page. A page break here may
affect the output.}
...
standard (see section \ref{preops}). Also with standard citations, the
preamble option \opt{idemtracker} shortens the author's name of a
work's first citation if the previous citation is of the same author
(see section \ref{preops}). The entry option \opt{noauth} omits the
author's name altogether (see section \ref{entryops}). And the
