% Last modified: Sat 13 Jul 2019 06:04:16 PM CDT
...
Like \cmd{smartcites} but, like \cmd{smartcite*}, suppresses the author's
position of a note.
...
Note that \cmd{textcite} doesn't enclose citations in parentheses,
since that's not always necessary, but does drop the final period that
ends most standard citations. Enclosing the command in
\cmd{mkbibparens} adds parentheses and switches internal parentheses
to brackets.
...
Like \cmd{textcite} but, like \cmd{cites}, allows you to cite multiple
sources with a \bibfield{prenote} and \bibfield{postnote} for each
source, along with a \bibfield{prenote} and \bibfield{postnote} for
the set.
