% Last modified: Sun 20 Jun 2021 04:07:55 PM CDT
...
same author (see section \ref{preamble.native}). The entry option
...
Section \ref{preamble.windy} discusses the style's own preamble
options. Section \ref{preamble.native} highlights a few options from
\biblatex for additional customization.

\subsubsection{Windy City's Preamble Options}
\label{preamble.windy}

All options below are \opt{false} by default. Set them to \opt{true} by
passing the name of the option to \biblatex, with or without an
additional \opt{=true}. In other words, using \opt{annotate} as an
...
\subsubsection{Native \biblatex Preamble Options}
\label{preamble.native}
...
with the user guide for \biblatex.

The options below are unique in that Windy City provides support for
them (they're not style-independent). Note that, unlike those in the
previous section, some of the options below are \opt{true} by default.

\begin{optionlist}

\optitem[true]{dashed}{\opt{true}, \opt{false}}

\noindent Set this option to \opt{false} to remove the 3-em dash for
repeated names in bibliographies and reference lists.

\optitem[true]{doi}{\opt{true}, \opt{false}}

\noindent Set this option to \opt{false} to suppress printing the
\bibfield{doi} field in notes, bibliographies, and reference lists.

\optitem[true]{eprint}{\opt{true}, \opt{false}}

\noindent Set this option to \opt{false} to suppress printing the
\bibfield{eprint} field in notes, bibliographies, and reference lists.
For more information about eprints, see section \ref{datafields}.

\optitem[false]{ibidpage}{\opt{true}, \opt{false}}

\noindent Set this option to \opt{true} to suppress printing the
\bibfield{postnote} field of a citation if the previous citation on
the page is to the same entry with the same \bibfield{postnote}. The
\opt{ibid} preamble option sets this option to \opt{true}.

\optitem[false]{idemtracker}{\opt{true}, \opt{false}, \opt{context},
\opt{strict}, \opt{constrict}}

\noindent If this option is \opt{true}, the first citation of a work
prints with a shortened author's name if the previous citation is of
...
the first citation of \textit{Beloved}. With \opt{idemtracker} set to
\opt{true} (or in many contexts to any value other than \opt{false}),
the style prints her full name just once, at the beginning of the
series.
...
\noindent Set this option to \opt{false} to suppress printing the
\bibfield{url} field in notes, bibliographies, and reference lists.
