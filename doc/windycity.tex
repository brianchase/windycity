% Last modified: Sun 17 Feb 2019 12:11:12 PM CST
...
articles, chapters, and books within books, this scheme allows for
several different levels of editors, although it does limit you to one
set of compilers per work.
...
many as three, if you cross-reference a title with a translator to a
collection with \bibfield{title} and \bibfield{maintitle} translators.
Does any such work exist? Probably not. Support for it is something of
an accident, stemming from how the style supports multiple
cross-references, as when you cross-reference an article to a volume
that cross-references a collection.
...
its output is formally correct:
...
This drawback shouldn't affect \bibfield{incollection} or
\bibfield{letter} entries when they're cross-referenced to works that
are changed by \opt{swapvol}. They should have their own authors or
titles that Windy City can use to sort the works correctly.
Regardless, \opt{swapvol} affects them, too. \textit{CMS} \ref{14.120}
gives an example:
...
work with \opt{swapvol}? Below are the entries for an earlier example:
...
A \bibtype{bookinbook} or \bibtype{inbook} entry works with
\opt{swapvol} if is has a \bibfield{volume} field and lacks certain
fields that a volume of a collection of this sort shouldn't have. You
can find the exact details in \file{windycity.bbx}. Just remember that
entries of these types should always be cross-referenced to a
\bibtype{collection} entry. Since Windy City treats them the same, it
makes no difference whether you designate an entry as
\bibtype{bookinbook} or \bibtype{inbook}.
...
equivalent. Here's a counterpart to the previous example using a
...
\bibfield{volume} and \bibfield{maintitle} fields. You may prefer them
over \bibfield{bookinbook} or \bibfield{inbook} entries if you only
intend to cite one volume of the collection and have no need to cite
the collection as a whole. You risk trouble, though, if the volumes of
the collection have different publication dates, as they do for
\textit{The Papers of Martin Luther King, Jr}. If the publication
...
An \bibtype{incollection} or \bibtype{letter} entry works with
\opt{swapvol} if it passes one or both tests for the previous types.
This means you may choose either approach mentioned above:
...
\noindent In other words, you may cross-reference King's letter to
...
\bibfield{title}, not \bibfield{maintitle}. The upshot is that
...
\textit{no.}, as \textit{CMS} has it. To print the correct season,
Wilder's entry in the bibliography database includes \textit{Fall} in
the \bibfield{issue} field. Using the \bibfield{date} field with
\textit{2013-23} would give \textit{Autumn 2013}.
...
section or edition of a newspaper or magazine. Thus, ``Sunday Book
Review'' is missing from the second example. It does, however, show
success at formatting a review with a title.
